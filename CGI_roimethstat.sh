...
## Takes 3 arguments: paths to the CGI annotation file for mouse and humans.
## and prefix for path to the repo
...
for i in $(cat human_pmdsamplenames); do qsub -v IN=${i}.meth,REF=${1} $3/do-roimethstat-islands.pbs; sleep 0.1; done

grep "Mouse" pmd_samplenames > mouse_pmdsamplenames
for i in $(cat mouse_pmdsamplenames); do qsub -v IN=${i}.meth,REF=${2} $3/do-roimethstat-islands.pbs; sleep 0.1; done
